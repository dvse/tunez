# One generated module per backend: ExUnit runs async modules
# CONCURRENTLY, so the LiveView and Datastar equivalence arms execute in
# parallel. Postgres isolates per test via the sandbox owner ($callers
# reaches LiveView processes); all UI/ETS state is session-scoped.
for backend <- [:phoenix, :datastar] do
  defmodule Module.concat(
              Tunez.UI,
              "InteractionEquivalence#{Macro.camelize(to_string(backend))}Test"
            ) do
    ...
    setup_all do
      %{upstream: upstream_oracle!()}
    end

    defp upstream_oracle! do
      :global.trans({:tunez_upstream_oracle, self()}, fn ->
        case :persistent_term.get(:tunez_upstream_oracle, nil) do
          nil ->
            oracle = Tunez.HTMLParity.render_upstream_screens!()
            :persistent_term.put(:tunez_upstream_oracle, oracle)
            oracle
    ...
    test "every catalogue document title matches its upstream screen", %{
      ...
      for {scenario, request_conn, path} <- [
        ...
        doc_path = if @backend == :datastar, do: "/ds" <> path, else: path
        blueprint = request_conn |> get(doc_path) |> Map.fetch!(:resp_body)
    ...
    defp submit_and_follow(%BackendView{backend: :phoenix} = view, selector, path) do
      ...
      Phoenix.LiveViewTest.assert_patch(view.view, path)

      %BackendView{
        view
        | html: Phoenix.LiveViewTest.render(view.view),
          path: path,
    ...
    defp submit_and_follow(%BackendView{backend: :datastar} = view, selector, path) do
      ...
      expected = "history.pushState({}, '', #{Jason.encode!(datastar_path(path))})"
    ...
    defp dispatch_binding(%BackendView{backend: :datastar} = view, binding, event, payload) do
      ...
      document_patch =
        Enum.find(traffic, fn patch ->
          view_patch?(patch) and is_nil(patch.selector) and
            String.contains?(patch.elements, "<html")
        end)

      if document_patch do
        assert Enum.any?(traffic, &navigation_effect?/1)

        after_state = AshBlueprint.Datastar.Store.get(view.session_id)
        canonical = render_datastar(view.session_id)

        document_root = datastar_root(document_patch.elements)
        canonical_root = datastar_root(canonical)

        assert document_root == canonical_root,
               "#{event} live navigation document differs from its fresh Ash mount: #{inspect(first_tree_difference(Floki.parse_fragment!(document_root), Floki.parse_fragment!(canonical_root), []), printable_limit: 2_000)}"

        live_client = LiveClient.merge_diff(%{}, after_state.live.diff)

        %{
          view
          | conn: response,
            html: document_patch.elements,
            live_client: live_client,
            last_response: response,
            traffic: traffic
        }
    ...
    defp binding_key!(view, selector, event, text \\ nil)

    defp binding_key!(%BackendView{backend: :phoenix} = view, selector, event, text) do
    ...
    defp binding_key!(%BackendView{backend: :datastar} = view, selector, event, text) do
      node = target_node!(view, selector, text)
      attribute = datastar_event_attribute(event)

      # Datastar's minimal DOM carries the binding address only in the dispatch URL.
      with [expression] <- Floki.attribute([node], attribute),
           [_, encoded] <-
             Regex.run(~r{/_ash_blueprint/dispatch/([^&'"),\s]+)}, expression) do
        URI.decode(encoded)
      else
        values ->
          raise "expected one dispatch URL in #{attribute} on #{selector}, got #{inspect(values)}"
      end
    end

    defp event_attribute_name(event), do: event |> to_string() |> String.replace("_", "-")

    defp datastar_event_attribute(:click_away), do: "data-on:click__outside"
    defp datastar_event_attribute(event), do: "data-on:" <> event_attribute_name(event)
    ...
    defp assert_datastar_traffic_equivalent!(patched, canonical, traffic, event) do
      ...
      assert patched == canonical,
             "#{event} Datastar SSE fragments do not reconstruct the canonical resource view: #{inspect(%{difference: first_tree_difference(Floki.parse_fragment!(patched), Floki.parse_fragment!(canonical), []), patches: Enum.map(Enum.filter(traffic, &view_patch?/1), &{&1.selector, String.slice(&1.elements, 0, 500)})}, printable_limit: 4_000)}"
    end
    ...
    defp navigation_effect?(%{type: "datastar-patch-elements", elements: elements}) do
      ...
      String.starts_with?(elements, "<script") and elements =~ "history.pushState"
    end
